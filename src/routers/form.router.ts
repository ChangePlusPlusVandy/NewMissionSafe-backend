...
import { createAndAddResponseToForm, createForm, getAllForms, getFormResponse } from "../controllers/form.controller";
...
formRouter.post("/", async (req, res) => {
    try {
        ...
        if (err instanceof HttpError) {
            ...
            res.status(500).json({ error: "An unknown error occurred while creating form" });
        }
    }
})
...
formRouter.get("/", async (req, res) => {
    try {
        ...
        if (err instanceof HttpError) {
            ...
            res.status(500).json({ error: "An unknown error occurred while getting all forms" });
        }
    }
})
...
formRouter.put("/formID/:formID", async (req, res) => {
    try {
        ...
        if (err instanceof HttpError) {
            ...
            res.status(500).json({ error: "An unknown error occurred while adding response" });
        }
    }
});
